...
#define IRQ_IDX_NONSECURE_PL1  0
#define IRQ_IDX_SECURE_PL1     1
#define IRQ_IDX_NONSECURE_PL2  2
#define IRQ_IDX_VIRTUAL        3
/* 
...
//Poll CTP_CTL_EL0
static void
__armv7_timer_set_timer_freq(unsigned int hz)
{
...
static struct arch_timer armv7_timer = {
	.name               = "ARMv7",
	.dt_node            = NULL,
	.core_init          = __armv7_timer_core_init,
	.set_timer_freq     = __armv7_timer_set_timer_freq
};


void
armv7_timer_init( struct device_tree * dt_node )
{
	...
	for (i = 0; i < 4; i++) {
		printk("Timer IRQ[%d] : %d\n", i, timer_irqs[i].vector);
	}
	...
	armv7_timer.dt_node = dt_node;
	arch_timer_register(&armv7_timer);
}
