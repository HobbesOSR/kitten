...
void walk_pts(void) {
    ...
    while (vaddr <= 0xffffffffffffffffULL) {
        ret = arch_aspace_virt_to_phys(NULL, vaddr, &paddr);
        ...
        if (ret >= 0) {
            printk("Kernel page table: (%p ---> %p, size %d)\n",
                (void *)vaddr, (void *)paddr, ret);
...
extern void PrintHostPageTables(void *, v3_cpu_mode_t, unsigned long long);
...
pisces_reserve_memory(void)
{
	...
	{
	    uintptr_t start = pisces_boot_params->initrd_addr;
	    uintptr_t size = pisces_boot_params->initrd_size;

	    if (start + size <= (end_pfn << PAGE_SHIFT)) {
		printk(KERN_INFO
        ...
        /* reverve for shared_info_t, including MP tables, APIC tables, console buffer */
        reserve_bootmem(__pa(pisces_shared_info), sizeof(*pisces_shared_info));
        printk(KERN_INFO
                "reserving memory used by pisces_shared_infos\n");
        printk(KERN_INFO
                "  start=0x%lx, size=%ld bytes\n",
                (unsigned long) __pa(pisces_shared_info),
                (unsigned long) sizeof(*pisces_shared_info));
...
{
        if (acpi_disabled) {
                printk(KERN_WARNING "ACPI is disabled, cannot initialize.\n");
                return;
        }
...
#if 0
    {
        u64 mem_start = pisces_boot_params->base_mem_paddr;
        u64 mem_end = mem_start + pisces_boot_params->base_mem_size;
        init_kernel_pgtables(mem_start, mem_end);
    }
#endif
...
//    printk("After pisces setup:\n");
//    PrintHostPageTables(NULL, 3, get_cr3());
//    walk_pts();
//    printk("..done\n");
